...
def tenant_editor(path):
    ...
    if 'Darwin' in system:
        for root, dir, files in os.walk(path):
            for file in files:
                if file == 'tenant.xml':
                    exportTime_arg = """ sed -i "" 's/\(exportTime=\)"[^"]*"/\\1" "/g' """ + "\"" + os.path.join(root,
                                                                                                                 file) + "\""
                    ...
                    owner_id_arg = """ sed -i "" 's/\(owner-id=\)"[^"]*"/\\1" "/g' """ + "\"" + os.path.join(root,
                                                                                                             file) + "\""
                    ...
                    Parent_arg = """ sed -i "" 's/\(parent=\)"[^"]*"/\\1" "/g' """ + "\"" + os.path.join(root,
                                                                                                         file) + "\""
        ...
        try:
            if export_file_path != None:
                ...
                for path in path_list:
                    ...
                    COUNT += 1
                temp_string = path_list[temp_start_index] + '_' + path_list[temp_start_index + 1] + '_' + path_list[
                    temp_start_index + 4]
                if set1 != set2:
                    # FILES ARE DIFFERENT
                ...
                try:
                    ...
                    print Exception("CANT CREATE FILE", log_name)
        except Exception:
            print "MISSING FILE"
            path_list = import_dictionary[key].split('/')
            temp_string = path_list[5] + '_' + path_list[6] + '_' + path_list[9] + '_' + 'NF' + '.dif'
                ...
                for dash_data in dashboard_cases:
                    if dash_data == 'dashboards':
                        ...
                        try:
                            ...
                            writeLogMessage('%s %s' % ("Unable to open Dashboard tenant", dash_data), mainLogger,
                                            'critical')
