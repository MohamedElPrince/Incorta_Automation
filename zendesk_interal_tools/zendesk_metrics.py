"""https://incorta.zendesk.com/api/v2/ticket_metrics.json"""
...
class ZendeskUser(object):
    """A Organization of Zendesk for Incorta Zendesk"""
    ...
    def get_user_id(self):
        """Get Singular User Id"""
        self.user_url = self.zendesk_url + '/api/v2/ticket_metrics/'
    ...
    def parser(self, data):
        """Parse all Ids"""
        max = len(data['ticket_metrics'])
        for count in range(0,max):
            self.user_id.append(data['ticket_metrics'][count]['id'])

    def dump_jsons(self):
        """Dump json for kafka consumer"""
        max = len(self.user_id)
        path = create_directory(ROOT_DIR,'json_metric')
        for counter in range(0,max):
            self.user_url = self.zendesk_url + '/api/v2/ticket_metrics/' + str(self.user_id[counter])
            response = self.session.get(self.user_url)
