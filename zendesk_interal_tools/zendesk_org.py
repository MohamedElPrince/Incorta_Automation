...
class ZendeskUser(object):
    ...
    def get_user_id(self):
        ...
        self.user_url = self.zendesk_url + '/api/v2/organizations/'
    ...
    def parser(self, data):
        """Parse all Ids"""
        max = len(data['organizations'])
        for count in range(0,max):
            self.user_id.append(data['organizations'][count]['id'])

    def dump_jsons(self):
        """Dump json for kafka consumer"""
        max = len(self.user_id)
        path = create_directory(ROOT_DIR,'json_org')
        for counter in range(0,max):
            self.user_url = self.zendesk_url + '/api/v2/organizations/' + str(self.user_id[counter])
            response = self.session.get(self.user_url)
