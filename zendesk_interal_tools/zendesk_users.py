...
class ZendeskUser(object):
    ...
    def get_user_id(self):
        ...
        self.user_url = self.zendesk_url + '/api/v2/users/'
    ...
    def parser(self, data):
        ...
        for count in range(0,max):
            self.user_id.append(data['users'][count]['id'])

    def dump_jsons(self):
        """Dump json for kafka consumer"""
        max = len(self.user_id)
        path = create_directory(ROOT_DIR,'json_users')
        for counter in range(0,max):
            self.user_url = self.zendesk_url + '/api/v2/users/' + str(self.user_id[counter])
            response = self.session.get(self.user_url)
